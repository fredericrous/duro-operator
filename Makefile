...
IMG ?= ghcr.io/fredericrous/duro-operator:$(VERSION)
...
docker-build: ## Build docker image with the manager.
	docker build -t ${IMG} .
	docker tag ${IMG} ghcr.io/fredericrous/duro-operator:latest

docker-push: ## Push docker image with the manager.
	docker push ${IMG}
	docker push ghcr.io/fredericrous/duro-operator:latest
...
# branch head, pinned as its pseudo-version so the build is reproducible; its
# own go directive is 1.24.0, comfortably below go.mod's.
